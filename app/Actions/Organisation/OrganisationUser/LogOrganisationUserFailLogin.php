...
class LogOrganisationUserFailLogin
{
    use AsAction;
    use WithLogRequest;

    public function handle(Organisation $organisation, array $credentials, string $ip, string $userAgent, Carbon $datetime): void
    {
        $organisationUser = OrganisationUser::where('username', Arr::get($credentials, 'username'))->first();

        $this->log($datetime, $ip, $userAgent, Arr::get($credentials, 'username'), $organisationUser?->id);
        ...
        $organisation->stats()->update(
            [
                'number_failed_logins' => $organisation->stats->number_failed_logins + 1,
                'last_failed_login_at' => $datetime
            ]
        );
    ...
    public function log(Carbon $datetime, string $ip, string $userAgent, string $username, ?int $organisationUserID): void
    {
