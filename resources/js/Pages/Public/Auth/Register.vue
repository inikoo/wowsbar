    ...
    <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="">
            <button type="submit"  id="submit"
                ...
                <FontAwesomeIcon icon="fad fa-spinner-third" class="ml-2 h-5 w-5 animate-spin opacity-0" :class="{'opacity-100': form.processing}" />
            </button>
        </div>
        ...
        <div class="">
            <div class="text-sm font-light text-gray-500">
                ...
                <Link :href="route('public.login')" id="login-link"
                    class="font-bold text-primary-700 hover:underline">{{ trans('Login') }}</Link>
            </div>
        </div>
    </form>
    </div>
