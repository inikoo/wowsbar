<!--
  ...
  - Created: Tue, 17 Oct 2023 18:22:25 Malaysia Time, Office, Bali, Indonesia
...
function userRoute(user: User) {
    return route(
        ...
        [route().params['shop'],route().params['customer'],user.slug]);
}
    ...
    <Table :resource="data" :name="tab" class="mt-5">
        ...
        <template #cell(rolesz)="{ item: user }">
            <div class="flex gap-x-1 flex-wrap gap-y-1">
                ...
                <Tag v-for="role in user.roles" :theme="setThemeRoles(role)" :key="user.id">
                    {{ role }}
                </Tag>
            </div>
        </template>
        <template #cell(roles)="{ item: user }">
            <div class="flex gap-x-1 flex-wrap gap-y-1">
                <Tag v-for="role in user['roles']"  :theme="setThemeRoles(role)" >{{role.name}}</Tag>
            </div>
        </template>
    </Table>
</template>
