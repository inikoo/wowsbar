...
watch(activeSidebar, (newSidebar: string) => {
    if(newSidebar == 'uploaded_images') {
        galleryStore[newSidebar].length === 0 ? getData(newSidebar, 'customer.portfolio.uploaded.images') : false
    } else if (newSidebar == 'stock_images') {
        galleryStore[newSidebar].length === 0 ? getData(newSidebar, 'customer.portfolio.stock.images') : false
    }
}, { immediate: true })
