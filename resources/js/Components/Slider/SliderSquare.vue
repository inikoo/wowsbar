...
<template>
    ...
    <!-- Square -->
    <div class="relative shadow overflow-hidden" :class="[
        ...
        <Swiper ref="swiperRef" :slideToClickedSlide="false" :spaceBetween="0" :slidesPerView="compSlidesPerView"
            ...
            <SwiperSlide v-for="component in compHandleBannerLessSlide" :key="component.id"
                ...
                <div v-if="get(component, ['backgroundType', $props.view ? $props.view : 'desktop'], 'image') === 'image'"
                    class="relative w-full h-full">
                    <Image :src="get(component, ['image', $props.view ? $props.view : 'desktop', 'source'], null)"
                        alt="Wowsbar" />
                </div>
                <div v-else
                    :style="{ background: get(component.background, $props.view ? $props.view : 'desktop', component.background?.desktop ?? 'gray') }"
                    class="w-full h-full" />
                ...
                <!-- <FontAwesomeIcon v-if="!!component?.layout?.link" icon='far fa-external-link' class='text-gray-300/50 text-xl absolute top-2 right-2' aria-hidden='true' /> -->
                <a target="_top" v-if="!!component?.layout?.link" :href="component?.layout?.link"
