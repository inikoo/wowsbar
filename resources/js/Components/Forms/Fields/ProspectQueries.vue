<!--
  ...
  - Created: Fri, 24 Nov 2023 18:05:45 Malaysia Time, Kuala Lumpur, Malaysia
    ...
    <div>
        <table class="min-w-full divide-y divide-gray-300 border-b border-gray-200 text-xs">
            ...
            <tr class="text-left text-sm font-semibold text-gray-600">
                ...
                <th scope="col" class="whitespace-nowrap px-2 pb-2.5 text-right">{{ trans('Prospects') }}</th>
                <th scope="col" class="relative whitespace-nowrap pb-2.5 pl-3 pr-4 sm:pr-0">
                    <span class="sr-only">{{ trans('Select') }}</span>
                </th>
            </tr>
            </thead>
            ...
            <tr v-for="option in options.data" :key="option.id" class=""
                ...
                <td class="px-2 py-2 text-right tabular-nums">{{locale.number( option.number_items)}}</td>
                <td class="relative py-2 px-3 text-right font-medium">
                    <div v-if="option.number_items > 0">
                        ...
                        <input v-model="form[fieldName].recipient_builder_data.query" :value="{id : option.id, arguments: option.arguments }" type="radio" :id="'radioProspects' + option.id" name="radioProspects"
